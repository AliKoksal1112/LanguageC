...
bool lireLivre (Livre *l) {
   ...
   if (!readLimitedLine(ligne,BUF_SIZE)) return false;
   strcpy(l->titre, ligne);
   // OU:  readLimitedLine(l->titre,MAX_TITRE+2)  mais struct Livre doit avoir des tailles de chaînes +2
...
void afficherBib (const Livre* bib, int t) {
   ...
   /*
     for (Livre *ptr = bib; ptr < bib+t; ptr++) {
        printf("\t%s\n", livre2str(ligne, *ptr));
     }
   */
   printf("---------------------------\n");
